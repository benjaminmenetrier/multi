...
\end{align}
\end{itemize}
Other ways to obtain the analysis increment at full resolution $\delta \mathbf{x}^{a+}_i$ lead to an inconsistency between the explicit guess and implicit guesses.
...
% 
% 
% Even if the strategy leads to inconsistent explicit and implicit guesses, both preconditionings can lead to the same results if and only if:
% \begin{enumerate}
% \item The explicit guess is the same, which requires a similar $\delta \mathbf{x}^{a+}_i$ for both preconditionings, computed either from %TODO...
% \item The implicit guess is the same, which means that the background increment implied by the first term of the right-hand side is similar for both preconditionings.
% \end{enumerate}
% 
% 
% 
% should be similar to the implicit guess included in the first term of the right-hand side can be different. To make sure that this implicit guess is also similar between both preconditionings (even if it is not consistent with the explicit guess), equation \eqref{eq:back_inc_Bvar} and \eqref{eq:back_inc_Uvar} have to yield the same background increment $\delta \mathbf{x}^b_k$:
% \begin{align}
%  \mathbf{B}_k \delta \underline{\overline{\mathbf{x}}}^b_k & = \mathbf{U}_k \delta \underline{\mathbf{v}}^b_k \nonumber \\
% \Leftrightarrow - \mathbf{B}_k \sum_{i=1}^{k-1} \mathbf{T}^\mathbf{x}_{i \rightarrow k} \delta \overline{\mathbf{x}}^a_i & = -\mathbf{U}_k \sum_{i=1}^{k-1} \mathbf{T}^\mathbf{v}_{i \rightarrow k} \delta \mathbf{v}^a_i \nonumber \\
% & = -\mathbf{U}_k \sum_{i=1}^{k-1} \mathbf{T}^\mathbf{v}_{i \rightarrow k} \mathbf{U}_i^\mathrm{T} \delta \overline{\mathbf{x}}^a_i
% \end{align}
% 
% 
% \subsubsection{For the alternative strategy}
%  implied by equations \eqref{eq:back_inc_Bvar} and \eqref{eq:back_inc_Uvar}:
% \begin{align}
