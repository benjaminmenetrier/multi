...
iter_params = itertools.product(i_no_ni, i_nx, i_nobs, i_sigma_obs,
                                ...
                                i_test_ortho, i_seed_count,repeat=1)
# Results directories:
res_dir_list=[]

#for seed_count in range(10):
if True:    

    for no_ni, nx, nobs, sigma_obs, Hnl_coeff, sigmabvar, Lb, interp_method, projective_Bmatrix, test_ortho, seed_count in iter_params:
        ...
        name_string = f'seed_{seed_count}'
        res_file_name += "_"+dir_name
        res_dir = os.path.join(res_dir + '/' +  name_string)
        if not os.path.exists(res_dir):
            os.mkdir(res_dir)
    ...
    ################################################################################
    # Analysis:
    print('Starting analysis:',seed_count)
    ################################################################################
        ...
        for met in ds.groups:
            ...
            for io in ds[met].groups:
                ...
                background_fields = ['sigmab', 'dirac_cov', 'dirac_cor', 'dirac_cov_bis',
                                     'dirac_cor_bis', 'xb']
                # To accelerate the code when just launching tests:
                background_fields=[]
